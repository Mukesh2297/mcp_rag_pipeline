...
class DocumentService():
    ...
    def __init__(self, documentRepository: DocumentRepository) -> None:
        print("Document Service injected")
        self.document_repository = documentRepository
    
    async def get_dense_search_results(self, query: str, top_k: int):
        return self.document_repository.get_dense_search_results(query=query, top_k=top_k)

    async def get_sparse_results(self, query: str, top_k = 20):
        return self.document_repository.get_sparse_search_results(query=query, top_k=top_k)
    ...
    async def get_response(self, query:str, top_k: int = 20):
        top_score_docs: list[dict] = self.hybrid_search(query=query, db=db, top_k=top_k)
        prompt = f"""
        {query}
